			...
			if(window.location.search==="")
			    window.location.pathname="/planter-rough/main.php";
				else 
			 {
			  var mruser = window.atob(window.location.search.slice(window.location.search.indexOf('=')+1));
				...
				//indow.open( dataUri, 'mywindow' );
				  ...
				  bal.onreadystatechange = function()
				  {
				    if(bal.readyState ==4 && bal.status ==200)
					  //document.write(bal.responseText);
					  ;
				  }
				  bal.send(vars);
			}
			...
			// console.log( world2.applyMatrix4(pvMatrix));
				 ...
				 function clean()
			 {
			  for(i=0;i<scene.children.length;i++)
				{
				if(scene.children[i].name !== 'voxel') continue;
				else
				{
				scene.remove(scene.__removeObject(scene.children[i]));
